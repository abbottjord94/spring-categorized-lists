	...
	app.controller("shoppingListCtrl", function($scope, $http) {
		...
		$scope.parseList = function(data) {
			$scope.items = data;
			$scope.categories = $scope.items.categoryList;
			//$scope.categories.sort();
			$scope.fixedCategories = [];
			for(x in $scope.categories) {
			    console.log($scope.categories[x]);
				$scope.fixedCategories.push({category: $scope.categories[x].categoryName, hash: "a" + $scope.hashCode($scope.categories[x].categoryName).toString(), num: 0});
			}
		}
		...
		$scope.showAddItemModal = function(category) {
			...
			for(var i = 0; i<addItemCategorySelector.options.length; i++) {
				...
				if(addItemCategorySelector.options[i].text === $scope.categories[category].categoryName) {
					addItemCategorySelector.options[i].selected = true;
					$scope.addItemCategory = $scope.categories[category].categoryName;
				}
			}
